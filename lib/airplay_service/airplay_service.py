...
class AirplayService(MusicSource):
    ...
    def __init__(self, start_binary: bool = True, pipe_path: str = PIPE_PATH) -> None:
        self.name = "airplay"
    ...
    def _dbus_worker(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                bus = dbus.SystemBus()
                remote = bus.get_object("org.gnome.ShairportSync", "/org/gnome/ShairportSync")
                interface = dbus.Interface(remote, "org.freedesktop.DBus.Properties")
                with self._lock:
                    self.properties_interface = interface
                delay = 1.0
                while not self._wait(1):
                    try:
                        interface.Get("org.gnome.ShairportSync.RemoteControl", "PlayerState")
    ...
    def get_play_state(self) -> bool:
        ...
        if interface is None:
            return False
        try:
            state = interface.Get("org.gnome.ShairportSync.RemoteControl", "PlayerState")

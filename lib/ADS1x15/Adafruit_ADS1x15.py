...
class ADS1x15:
    ...
    __ADS1015_REG_CONFIG_DR_1600SPS = 0x0080
    __ADS1015_REG_CONFIG_DR_2400SPS = 0x00A0  # 2400 samples per second
    # 3300 samples per second (also 0x00E0)
    __ADS1015_REG_CONFIG_DR_3300SPS = 0x00C0
    ...
    def __init__(self, address=0x48, ic=__IC_ADS1015, debug=False, i2c=None, busnum=1):
        ...
        if i2c is None:
            i2c = ai2c.Adafruit_I2C(address, busnum=busnum, debug=debug)
        self.i2c = i2c
        ...
        if ((ic < self.__IC_ADS1015) | (ic > self.__IC_ADS1115)):
            logger.debug(f"ADS1x15: Invalid IC specfied: {ic:02x}")
            return -1
        else:
            self.ic = ic
        ...
        self.pga = 6144
        logger.debug(f"ADS15 Init")

    def readADCSingleEnded(self, channel=0, pga=6144, sps=50):
        """Gets a single-ended ADC reading from the specified channel in mV. \
        The sample rate for this mode (single-shot) can be used to lower the noise \
        (low sps) or to lower the power consumption (high sps) by duty cycling, \
        see datasheet page 14 for more info. \
        The pga must be given in mV, see page 13 for the supported values."""
        logger.debug(f"readADCSingleEnded {channel}, {pga}, {sps}")

        # With invalid channel return -1
        if (channel > 3):
            logger.debug(f"ADS1x15: Invalid channel specified: {channel:d}")
            return -1
        ...
        if (self.ic == self.__IC_ADS1015):
            config |= self.spsADS1015.setdefault(
                sps, self.__ADS1015_REG_CONFIG_DR_1600SPS)
        else:
            if ((sps not in self.spsADS1115)):
                logger.debug(f"Invalid pga specified: {sps:d} using 6144mV")
            config |= self.spsADS1115.setdefault(
                sps, self.__ADS1115_REG_CONFIG_DR_250SPS)

        # Set PGA/voltage range, defaults to +-6.144V
        if (pga not in self.pgaADS1x15) :
            logger.debug("ADS1x15: Invalid pga specified: %d, using 6144mV" % sps)
    ...
    def readADCDifferential(self, chP=0, chN=1, pga=6144, sps=250):
        ...
        if ((chP == 0) & (chN == 1)):
            ...
            logger.debug("ADS1x15: Invalid channels specified: %d, %d" % (chP, chN))
            return -1
    ...
    def startContinuousConversion(self, channel=0, pga=6144, sps=250):
        ...
        if (channel > 3):
            logger.debug("ADS1x15: Invalid channel specified: %d" % channel)
            return -1
    ...
    def startSingleEndedComparator(self, channel, thresholdHigh, thresholdLow,
        ...
        # othewise it returns the value for 250sps. This saves a lot of if/elif/else code!
        if (self.ic == self.__IC_ADS1015):
            if (sps not in self.spsADS1015):
                logger.debug("ADS1x15: Invalid sps specified: %d, using 1600sps" % sps)
            config |= self.spsADS1015.setdefault(
                sps, self.__ADS1015_REG_CONFIG_DR_1600SPS)
        else:
            if (sps not in self.spsADS1115):
                logger.debug("ADS1x15: Invalid sps specified: %d, using 250sps" % sps)
            config |= self.spsADS1115.setdefault(
                sps, self.__ADS1115_REG_CONFIG_DR_250SPS)

        # Set PGA/voltage range, defaults to +-6.144V
        if (pga not in self.pgaADS1x15):
            logger.debug("ADS1x15: Invalid pga specified: %d, using 6144mV" % pga)
        config |= self.pgaADS1x15.setdefault(
            pga, self.__ADS1015_REG_CONFIG_PGA_6_144V)
        self.pga = pga
